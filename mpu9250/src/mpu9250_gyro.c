...
static esp_err_t mpu9250_gyro_load_default_calibration_params(
	...
	mpu9250_handle->data.gyro.cal.variances[X_POS] = 0.0;
	mpu9250_handle->data.gyro.cal.variances[Y_POS] = 0.0;
	mpu9250_handle->data.gyro.cal.variances[Z_POS] = 0.0;
	return ESP_OK;
}
...
esp_err_t mpu9250_gyro_load_calibration_params(
    ...
    memcpy(mpu9250_handle->data.gyro.cal.variances, (blob+48), 12);
    ...
    ESP_LOGI(TAG, "Gyro variances:\n [%5.5f,%5.5f,%5.5f]\n",
	   		mpu9250_handle->data.gyro.cal.variances[X_POS],
	   		mpu9250_handle->data.gyro.cal.variances[Y_POS],
	   		mpu9250_handle->data.gyro.cal.variances[Z_POS]
    		);
    return ESP_OK;
}

esp_err_t mpu9250_gyro_save_calibration_params(mpu9250_handle_t mpu9250_handle, char* data, int data_len) {
    if (data_len == 60) {  // 3x3 matrix + 3x1 bias = 15 floats = 15 * sizeof(float) = 15x4 = 60 bytes
...
esp_err_t mpu9250_gyro_load_cal_data(mpu9250_handle_t mpu9250_handle) {
	float biased_x = ((float)mpu9250_handle->data.raw_data.data_s_xyz.gyro_data_x/(float)mpu9250_handle->data.gyro.lsb - mpu9250_handle->data.gyro.cal.offsets[X_POS]);
	float biased_y = ((float)mpu9250_handle->data.raw_data.data_s_xyz.gyro_data_y/(float)mpu9250_handle->data.gyro.lsb - mpu9250_handle->data.gyro.cal.offsets[Y_POS]);
	float biased_z = ((float)mpu9250_handle->data.raw_data.data_s_xyz.gyro_data_z/(float)mpu9250_handle->data.gyro.lsb - mpu9250_handle->data.gyro.cal.offsets[Z_POS]);
	mpu9250_handle->data.cal_data.data_s_xyz.gyro_data_x = mpu9250_handle->data.gyro.cal.factors[X_POS][X_POS]*biased_x + mpu9250_handle->data.gyro.cal.factors[X_POS][Y_POS]*biased_y + mpu9250_handle->data.gyro.cal.factors[X_POS][Z_POS]*biased_z;
	mpu9250_handle->data.cal_data.data_s_xyz.gyro_data_y = mpu9250_handle->data.gyro.cal.factors[Y_POS][X_POS]*biased_x + mpu9250_handle->data.gyro.cal.factors[Y_POS][Y_POS]*biased_y + mpu9250_handle->data.gyro.cal.factors[Y_POS][Z_POS]*biased_z;
	mpu9250_handle->data.cal_data.data_s_xyz.gyro_data_z = mpu9250_handle->data.gyro.cal.factors[Z_POS][X_POS]*biased_x + mpu9250_handle->data.gyro.cal.factors[Z_POS][Y_POS]*biased_y + mpu9250_handle->data.gyro.cal.factors[Z_POS][Z_POS]*biased_z;
	return ESP_OK;
}

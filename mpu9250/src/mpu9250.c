...
esp_err_t mpu9250_init(mpu9250_handle_t mpu9250_handle) {
	// FIXME: per evitare che venga azzerato da spi_init che esegue
	//             memset(mpu9250_handle, 0, sizeof(mpu9250_init_t));
    //        togliere l'azzeramento ed eseguirlo prima di invocare init.
    ...
    // FIXME: togliere appena sistemato l'indicazione sopra
    mpu9250_handle->data.nvs_cal_data = nvs_handle;
